...
void cal_anm(Complex & complex, ANM & anm, int n_modes, double rmsd, int seed){
    ...
    int expct_n_zeros = 6;
    bool reverse = false;
    calModes(anm,n_modes,zeros,turbo,expct_n_zeros,reverse,n_atoms);
    ...
    // print_array(anm.eigvals,n_modes);

    // std::cout<< complex.residue_size[0]<<" "<< complex.residue_size[1]<<std::endl;
    vector <string> residues;
    // 具有相同res id 的索引
    // vector <vector<double>> res_real_indices;
    // std::cout<<complex.num_atoms<<std::endl;
    // step 1: 
    // 构建 atom index：具有相同的res原子索引集
    // 构建 node index：每个node都的原子索引
    // indices: nma的数量
    ...
    double * randn = new double[DEFAULT_NUM_SWARMS*DEFAULT_NUM_GLOWWORMS*anm.n_modes];
    double coef = 0;

    unsigned int seeds = 324324;
    generator.seed(seeds);

    for(int i = 0; i<DEFAULT_NUM_SWARMS*DEFAULT_NUM_GLOWWORMS; i++){
        double sum_ = 0;
    ...
    coef =  coef/ (DEFAULT_NUM_SWARMS*DEFAULT_NUM_GLOWWORMS);
    ...
    // print_mat(evec_buffer, complex.num_atoms*3, anm.n_modes);
    // transpose
...
void calModes(ANM  &anm, int n_modes, bool zeros, bool turbo, int expct_n_zeros, bool reverse, int n_atoms){
    /*
    anm.eigvals : vals;
    anm.eigvecs : vectors;
    anm.invvals : vars;
    */
    ...
    // string f = "mat";
    // f.assign(times);
    // write_mat(anm.hessian,dof,dof,f);
    // std::cout<<anm.hessian[0]<<std::endl;
    eigh_gpu(eigh_value,eigh_vector,anm.hessian,eigvals,dof);
    // std::cout<<"dof"<<dof<<std::endl;
    // std::cout<<"dof"<<length<<std::endl;
    // print_mat(eigh_vector, dof, length);
    // std::cout<<111111111<<eigh_value[99]<<std::endl;
    int n_zeros = 0;
    ...
    if(!zeros){
        if(n_zeros > expct_n_zeros){
            if (n_zeros == n_modes + expct_n_zeros && n_modes < dof){
...
void calculate_surface_point(
    ...
    double fixed_distance, int swarms_per_restraint, bool dense_smapling,  const char * rec_name, int number_of_points
){
    ...
    std::cout<<coord_size<<std::endl;
    // print_mat(surface_coord,coord_size,3);
    // TODO: rastraints

    // std::cout<<swarms<<std::endl;
    if (swarms==0){
        if (membrane){
            std::cout<<"developing"<<std::endl;
        }else{
    ...
    double * surface_centroids;
    surface_centroids = new double[swarms*3];
    int * group = new int[coord_size];
    // print_mat(receptor.atom_coordinates,receptor.structure_size[1],3);
    // print_mat(surface_coord,coord_size,3);
    // Surface clusters
    if(coord_size>swarms){
        ...
        std::cout<<"developing swarm size = coord size"<<std::endl;
        exit(0);
    }
...
void calculate_start_pos(
    ...
    int anm_rec, int anm_lig, bool membrane, bool transmambrane,
    bool write_starting_positions, double swarm_radius, bool flip,
    double fixed_distance, int swarms_per_restraint, bool dense_smapling, const char * rec_name
){
    ...
    std::mt19937 gen(start_points_seed); //Standard mersenne_twister_engine seeded with rd()
    ...
    // 
    // std::cout<<receptor.coord_size<<std::endl;
    ...
    rec_translation,  lig_translation,
     ...
     fixed_distance,  swarms_per_restraint,  dense_smapling,  rec_name,  number_of_points
    );
    ...
    // save center pdb
    // std::cout<<center.swarms<<std::endl;
    // string save_swarm = "swarm_";
    // string file_name = "center";
    // string file_type = ".pdb";
    // string save_dir =  save_swarm;
    // string save_file = file_name+file_type;
    // // std::cout<<save_file<<std::endl;
    // string slash = "/";
    // string full_path = save_file;
    // // TODO: 写文件
    // std::ofstream newFile(full_path.c_str());
    // write_center(center.s,newFile,center.swarms);
    // // write_pdb(lignad,newFile,current_lig_copy);
    // newFile.close();
    // exit(-1);
    // print_array(&center.pos[0],27);
