...
void cal_anm(Complex & complex, ANM & hessian_type, int n_modes, double rmsd, int seed);
...
void eigh_gpu(double * eigenvalues, double * eigenvectors, double * matrix,int *eigvals, int n_atoms);

void calModes(ANM  &hessian, int n_modes, bool zeros, bool turbo, int expct_n_zeros, bool reverse, int n_atoms);
...
void calculate_start_pos(
    ...
    double fixed_distance, int swarms_per_restraint, bool dense_smapling,const char * name
);
